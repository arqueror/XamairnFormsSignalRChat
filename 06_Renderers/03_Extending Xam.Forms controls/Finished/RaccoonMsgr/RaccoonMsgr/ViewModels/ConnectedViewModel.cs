...
namespace RaccoonMsgr.ViewModels
{
    public class ConnectedViewModel : BaseViewModel
    {
        ...
        public ConnectedViewModel(INavigation navigation, string pageTitle) : base(navigation, pageTitle)
        {
            ...
            DeleteConversationCommand = new Command<Contact>(async (conversation) => await DeleteConversationAsync(conversation));
            ...
            if (messages.Count > 0)
            {
                ...
                messages.ForEach(m =>
                {
                    ...
                    else
                    {
                        ...
                        var alreadyInList = tmpCollection.Where(u => u.Name == currModel.Name).FirstOrDefault();
        ...
        private async Task DeleteConversationAsync(Contact conversation)
        {

            var accepted = await Application.Current.MainPage.DisplayAlert("Delete", "Delete conversation?", "Yes", "Cancel");
            if (accepted)
            {
                Application.Current.MainPage.DisplayAlert("", "", "das");
            }
        }
